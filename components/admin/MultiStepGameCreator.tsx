...
const STEPS = [
  { id: 1, title: 'Classes grammaticales', description: 'Sélectionner les classes à utiliser' },
  { id: 2, title: 'Image', description: 'Choisir une image pour le poème' },
  { id: 3, title: 'Vers', description: 'Saisir le vers du poème' },
  { id: 4, title: 'Classification', description: 'Attribuer les classes aux mots' },
...
export default function MultiStepGameCreator({
    ...
    const loadWordClasses = async () => {
      if (editingPoem && session) {
        try {
          ...
          if (sessionToken) {
            const wordClasses = await PoemService.fetchWordClasses(sessionToken, editingPoem.id)
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      ...
      case 6:
        return formData.targetWordGender === 'masculin' || formData.targetWordGender === 'féminin'
      default:
        return true
    }
  }
  ...
  const handleNext = () => {
    if (canGoNext()) {
      ...
      toast.error(`Veuillez compléter l'étape "${stepTitle}" avant de continuer`)
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Save word classes
      await PoemService.saveWordClasses(sessionToken, poem.id, formData.wordClasses)
      ...
      case 1:
        return (
          <WordClassSelection
            ...
            onClassesChange={(classes: string[]) => updateFormData({ wordClasses: classes })}
          />
        )
      case 2:
        return (
      ...
      case 4:
        return (
          <WordClassification
            ...
            onWordColorsChange={(wordColors: {[key: number]: string}) => updateFormData({ wordColors })}
          />
        )
      case 5:
        return (
          <SecretWordInput
            ...
            words={formData.words}
            wordColors={formData.wordColors}
            onTargetWordChange={(targetWord: string) => updateFormData({ targetWord })}
          />
        )
      case 6:
        return (
          <GenderSelection
            targetWordGender={formData.targetWordGender}
            onGenderChange={(targetWordGender: 'masculin' | 'féminin') => updateFormData({ targetWordGender })}
          />
        )
      default:
        return null
